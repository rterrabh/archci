		...
		while (lnr.ready()) {
			...
			try {
				if (line.startsWith("%")) {
					...
					String action = moduleDescriptionB.substring(moduleDescriptionB.lastIndexOf("--"));
					action = moduleDescriptionB.substring(0, moduleDescriptionB.indexOf(" "));
					moduleDescriptionB = moduleDescriptionB.substring(0, moduleDescriptionB.lastIndexOf("--"));

					if(action.equals("debt")) warning=true;
					
					for (String restriction : restrictions.split(",")) {
						Constraint c = Constraint.getConstraint(restriction);
						dependencyConstraints.add(new DependencyConstraint(moduleDescriptionA, moduleDescriptionB, c, warning));
					}
				}
			} catch (Exception e) {
