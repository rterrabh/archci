...
public class Architecture {
	...
	public Architecture(FilePath projectPath) throws CoreException, ParseException, IOException, DCLException, InterruptedException {
		if (DEBUG) {
		...
		for (FilePath f : DCLUtil.getFilesFromProject(projectPath)) {
	...
	/*public Architecture(FilePath projectPath, //LinkedList<String> //classNames String className) throws CoreException, ParseException, IOException, DCLException, InterruptedException {
		if (DEBUG) {
			System.out.println("Time BEFORE generate architecture (without dependencies): " + new Date());
		}
		this.projectClasses = new HashMap<String, Collection<Dependency>>();
		...
		//for (FilePath f : DCLUtil.getFilesFromProjectAndClass(projectPath, classNames)) {
		
		FilePath f = DCLUtil.getFilesFromProjectAndClass(projectPath, className);
		if(f!=null){
			DCLDeepDependencyVisitor ddv = DCLUtil.useAST(f, classPathEntries, sourcePathEntries);
			this.projectClasses.put(ddv.getClassName(), ddv.getDependencies());
			this.typeBindings.add(ddv.getITypeBinding());
	...
	private void initializeDependencyConstraints(FilePath dclFile) throws CoreException, ParseException {
		try {
			if(dclFile!=null){
				//File dcl = new File(dclFile.getAbsolutePath());
				//InputStream inputStream = new FileInputStream(dcl);
				InputStream inputStream = dclFile.read();
				...
				InputStream inputStream2 = dclFile.read();
				
				this.modules.putAll(DCLParser.parseModules(inputStream2));
	...
	public void updateDependencyConstraints(FilePath projectPath) throws CoreException, ParseException {
		this.modules.clear();
